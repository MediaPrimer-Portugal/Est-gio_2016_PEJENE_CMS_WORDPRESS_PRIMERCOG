        ...
        public static function generateShortcode($atts=array()) {
            ...
            if ($_POST['bs_form_'.$formid]) {
                ...
                if($hasCaptcha) {
                    ...
                    if (strlen($_POST['g-recaptcha-response']) > 0) {
                        ...
                        if (isset($captcha) && $captcha) {
                            $response = json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=" . $settings['security']['recaptcha_secrete_key'] . "&response=" . $captcha . "&remoteip=" . $_SERVER['REMOTE_ADDR']), true);
                ...
                if (is_array($err) && isset($err) && $err) {
                    ...
                    // Email
                    $email = $formData[0]['form_email'];
                    $emailCC = $formData[0]['form_emailCC'];

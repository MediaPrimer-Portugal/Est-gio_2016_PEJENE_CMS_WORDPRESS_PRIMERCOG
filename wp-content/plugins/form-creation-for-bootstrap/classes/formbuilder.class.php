...
class BS_Forms_Builder {
    ...
    public function getInputType($type, $key, $d=array()) {
        ...
        $return .= '<div class="box-header">
                        ...
                        <button data-original-title="Remove" type="button" class="btn btn-default btn-sm" data-widget="remove">
        ...
        if ($this->formTypes[$type]['have_name']) {
            ...
            $return .= '<label for="form[' . $key . '][data_name]">Edit Name (Required)</label><input id="form[' . $key . '][data_name]" value="' . $d['data_name'] . '" class="form-control input" placeholder="Input Name" name="form[' . $key . '][data_name]"><small>Although not displayed, this is needed in order for a form to save the save. Must not contain spaces e.g first_name</small>';
            $return .= '</div>';
        }

        // Label
        if ($this->formTypes[$type]['have_label']) {
            $return .= '<label for="form[' . $key . '][data_label]">Edit Label</label><input id="form[' . $key . '][data_label]" value="' . $d['data_label'] . '" class="form-control input" placeholder="Input Label" name="form[' . $key . '][data_label]">';
        }

        // Class
        if ($this->formTypes[$type]['have_class']) {
            $return .= '<label for="form[' . $key . '][data_class]">Edit Class</label><input id="form[' . $key . '][data_class]" value="' . $d['data_class'] . '" class="form-control input" placeholder="Input Class" name="form[' . $key . '][data_class]">';
        }

        // Placeholder
        if ($this->formTypes[$type]['have_placeholder']) {
            $return .= '<label for="form[' . $key . '][data_placeholder]">Edit Placeholder</label>';
            $return .= '<input id="form[' . $key . '][data_placeholder]" value="' . $d['data_placeholder'] . '" class="form-control input placeholder="Input Placeholder" name="form[' . $key . '][data_placeholder]">';
        }
    ...
    public function generateColumn($input_id, $data) {
        ...
        $return .= '<select id="form[' . $input_id . '][data_column_xs]" class="form-control input" name="form[' . $input_id . '][data_column_xs]">' . $data[0] . '</select>';
        ...
        $return .= '<select id="form[' . $input_id . '][data_column_sm]" class="form-control input" name="form[' . $input_id . '][data_column_sm]">' . $data[1] . '</select>';

        $return .= '<label for="form[' . $input_id . '][data_column_md]">Choose Small Desktop Size</label>';
        $return .= '<select id="form[' . $input_id . '][data_column_md]" class="form-control input" name="form[' . $input_id . '][data_column_md]">' . $data[2] . '</select>';

        $return .= '<label for="form[' . $input_id . '][data_column_lg]">Choose Desktop Size</label>';
        $return .= '<select id="form[' . $input_id . '][data_column_lg]" class="form-control input" name="form[' . $input_id . '][data_column_lg]">' . $data[3] . '</select>';

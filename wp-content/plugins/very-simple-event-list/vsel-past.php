	...
	$output .= '<div id="vsel">'; 
		...
		if ( $vsel_events->have_posts() ) : 
			...
			$output .= '<div class="vsel-content">'; 
				$output .= '<div class="vsel-meta">'; 
					...
					if ($event_date_hide != 'yes') {
						if (!empty($event_start_date)) {
							if ($widget_date == 'yes') {
								$output .= '<p class="vsel-meta-date hide">';
							} else {
								$output .= '<p class="vsel-meta-date">';
							}
							if ($event_start_date >= $event_date) {
								$output .= sprintf(esc_attr($vsel_atts['date_label']), date_i18n( get_option( 'date_format' ), esc_attr($event_start_date) ) ); 
							} else {
					...
					if(!empty($event_time)){
						if ($widget_time == 'yes') {
							$output .= '<p class="vsel-meta-time hide">';
						} else {
					...
					if(!empty($event_location)){
						if ($widget_location == 'yes') {
							$output .= '<p class="vsel-meta-location hide">';
						} else {
					...
					if(!empty($event_link)){
						if ($widget_link == 'yes') {
							$output .= '<p class="vsel-meta-link hide">';
						} else {
				...
				if ($widget_info == 'yes') {
					$output .= '<div class="vsel-info hide">';
				} else {
					$output .= '<div class="vsel-info">';
				}
					if ( has_post_thumbnail() ) { 
						if ($widget_image == 'yes') {
							$output .= get_the_post_thumbnail( null, 'post-thumbnail', array('class' => 'vsel-image hide') ); 
						} else {
			...
			// pagination
			$output .= '<div class="vsel-nav hide">';
